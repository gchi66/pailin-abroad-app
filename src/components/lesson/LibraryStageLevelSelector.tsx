...
export function LibraryStageLevelSelector({
  language, stage, stages, level, levels, stageOpen,
  onToggleStage, onSelectStage, onSelectLevel, freeOnly = false, bottomMargin,
}: Props) {
  ...
  return (
    <View style={[styles.navigationShadow, bottomMargin === undefined ? null : { marginBottom: bottomMargin }]}>
        ...
        {!freeOnly ? (
          <Pressable accessibilityRole="button" accessibilityLabel={th ? 'เลือกช่วงการเรียน' : 'Choose stage'} accessibilityState={{ expanded: stageOpen }} onPress={onToggleStage} style={styles.stageHeader}>
            <View style={styles.stageHeading}>
              ...
              <AppText language={language} variant="caption" style={styles.stageName}>{stageLabel(stage)}</AppText>
            </View>
        ...
        {stageOpen || freeOnly ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.stages}>
            {LIBRARY_STAGES.filter((value) => stages.includes(value)).map((value) => (
              <Pressable key={value} accessibilityRole="button" accessibilityState={{ selected: value === stage }} onPress={() => onSelectStage(value)} style={styles.stageTouch}>
                <View style={[styles.stagePill, value === stage ? styles.activeStage : null]}>
                  <AppText language={language} variant="caption" style={[styles.stageName, value === stage ? styles.activeStageText : null]}>{stageLabel(value)}</AppText>
                </View>
              </Pressable>
            ))}
          </ScrollView>
...
const styles = StyleSheet.create({
  navigationShadow: { alignSelf: 'stretch', backgroundColor: '#222222', borderRadius: 10, marginBottom: 24, marginRight: -3, marginLeft: 3 },
  ...
  stageName: { fontSize: 12, lineHeight: 18, letterSpacing: 0.65, textTransform: 'uppercase' },
  stages: { flexGrow: 1, justifyContent: 'space-between', gap: 4, paddingHorizontal: 4, paddingTop: 4, paddingBottom: 4 },
  stageTouch: { padding: 4, justifyContent: 'center', minHeight: 32 },
  stagePill: { borderRadius: 4, backgroundColor: '#EEEEEE', paddingHorizontal: 5, paddingVertical: 2 },
  activeStage: { backgroundColor: '#2860F0' },
  activeStageText: { color: '#FFFFFF' },
